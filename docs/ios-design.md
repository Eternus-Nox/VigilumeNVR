...
- **Events list:** `GET /api/events?camera=&label=&after=&before=&limit=&offset=` →
  `{events, total}`; snapshots/clips via `?token=` URLs (§2.4). Events are
  ONE per camera (see CONTRACTS.md, "One event per camera"): an event's
  `labels` lists every type seen while it was open, and `label` is the most
  important of them. Rows and tiles show `allLabels`; the client does no
  grouping of its own.
