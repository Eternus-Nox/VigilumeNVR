...
/// Near-simultaneous detections on one camera, shown as ONE row.
///
/// The server opens one event per object type per camera, so a person and the
/// car they arrived in are two events a second apart, and the list used to show
/// two rows for one thing that happened. The list now shows one row for the
/// moment and the event screen lists the others ("Also detected at this
/// moment"). Presentation only: every record keeps its own clip, snapshot and
/// recognitions on the server.
///
/// Mirrors `frontend/src/lib/groupEvents.ts`; the rules are tested there by
/// `frontend/scripts/check-event-grouping.mjs`, so change both together.
struct EventGroup: Identifiable, Equatable, Sendable {
    /// The event the row shows: one with a clip if any member has one, else one
    /// with a snapshot, else the newest. Landing on "no recording was saved"
    /// when a sibling has a clip would make the moment look worse-recorded than
    /// it was.
    let lead: Event
    /// Every event in the group, in list order. One element for a lone event.
    let events: [Event]
    /// Distinct labels across the group, in list order, for the row's title.
    let labels: [String]

    /// The FIRST member's id rather than the lead's: the lead can change when
    /// a sibling's clip lands, and the row should update in place.
    var id: Int { events[0].id }

    /// A group of one — what every row is when grouping is off.
    init(single event: Event) {
        self.init(lead: event, events: [event], labels: event.allLabels)
    }

    init(lead: Event, events: [Event], labels: [String]) {
        self.lead = lead
        self.events = events
        self.labels = labels
    }
...
enum EventGrouping {
    ...
    /// moment: long enough for a person and the car they arrived in, short
    /// enough that two separate visits never merge. Same as the web.
    static let windowSeconds: Double = 10

    /// UserDefaults key for the on/off switch on the Events tab. On by default.
    static let preferenceKey = "vigilume.groupEvents"

    /// Group an ALREADY-SORTED list (newest first, as the API returns it).
    ///
    /// Same camera only — the same person on two cameras is evidence about where
    /// they went. Membership is measured from the group's FIRST event, not the
    /// previous member, so a slow trickle cannot chain into one huge group.
    /// Group order follows the first member, so turning grouping on never
    /// reshuffles the list.
    static func group(_ events: [Event]) -> [EventGroup] {
        var building: [(start: Double, events: [Event], labels: [String])] = []
        // Per camera, the index of the group currently open for it.
        var open: [String: Int] = [:]

        for event in events {
            if let index = open[event.camera],
               abs(building[index].start - event.startTime) <= windowSeconds {
                building[index].events.append(event)
                for label in event.allLabels where !building[index].labels.contains(label) {
                    building[index].labels.append(label)
                }
                continue
            }
            open[event.camera] = building.count
            building.append((start: event.startTime, events: [event], labels: event.allLabels))
        }
        return building.map { entry in
            EventGroup(lead: pickLead(entry.events), events: entry.events, labels: entry.labels)
        }
    }

    /// Best member to stand for the moment: clip, then snapshot, then the first.
    static func pickLead(_ events: [Event]) -> Event {
        events.first(where: { $0.hasClip })
            ?? events.first(where: { $0.hasSnapshot })
            ?? events[0]
    }
}
